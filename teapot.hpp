...
#include <glm/trigonometric.hpp> // for atan()
...
#define TEAPOT_RES_V 10
    ...
    inline Teapot_Vertex teapot_compute_texture(Teapot_Vertex position) {
        Teapot_Vertex result = {0.0f, 0.0f, 0.0f};
        GLfloat theta = glm::atan(position.y, position.x);
        result.x = (theta+PIf) / (2.0f*PIf);
        result.y = position.z / PIf;
        return result;
    }

    inline GLfloat teapot_bernstein_polynomial(int i, int n, GLfloat u) {
        return teapot_binomial_coefficient(i, n) * glm::pow(u, (GLfloat)i) *
               glm::pow(1.f - u, (GLfloat)(n - i));
    }
